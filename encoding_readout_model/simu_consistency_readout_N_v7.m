function out=simu_consistency_readout_N_v7(N,n_samples,mu_s0,mu_s1,stdev1_s0,stdev2_s0,stdev1_s1,stdev2_s1,rho_s0,rho_s1,rho_within,decoder_type,fitting_fraction,...
    alpha_readout,readout_strength,readout_strength_params,consistency_thr,equalization_type,display_figures)
...
sigma_s0 = [rho_s0*stdev1_s0*stdev2_s0*ones(N,N)].*(1-eye(N));
sigma_s0 = diag([(stdev1_s0^2)*ones(N,1)])+sigma_s0;%/(N);
sigma_s1 = [rho_s1*stdev1_s1*stdev2_s1*ones(N,N)].*(1-eye(N));
sigma_s1 = diag([(stdev1_s1^2)*ones(N,1)])+sigma_s1;%/(N);

sigma_sh_s0 = zeros(N,N);
sigma_sh_s0 = diag([(stdev1_s0^2)*ones(N,1)])+sigma_sh_s0;%/(N);
sigma_sh_s1 = zeros(N,N);
sigma_sh_s1 = diag([(stdev2_s1^2)*ones(N,1)])+sigma_sh_s1;%/(N);
% sigma_sh_s0 = diag([(stdev1_s0^2)*ones(N,1); (stdev2_s0^2)*ones(N,1)]);%/(N);
% sigma_sh_s1 = diag([(stdev2_s1^2)*ones(N,1); (stdev2_s1^2)*ones(N,1)]);%/(N);
...
%     r_all_sh(this_class_trials,N+1:end) = datasample(r_all(this_class_trials,N+1:end),nnz(this_class_trials),'Replace',false);
    for idx_n = 1:size(r_all_sh,2)
        r_all_sh(this_class_trials,idx_n) = datasample(r_all(this_class_trials,idx_n),nnz(this_class_trials),'Replace',false);
    end
...
switch decoder_type
    ...
    case 'analytical'
        ...
        f_db = @(x,y,z) L(1)*x+L(2)*y+L(3)*z+K;
        f_db_sh = @(x,y,z) L_sh(1)*x+L_sh(2)*y+L(3)*z+K_sh;
    ...
    case 'lda'
        ...
        posterior_probs_marg = zeros(length(test_idxs),size(r_all,2),2);
        for idx_n = 1:size(r_all,2)
            [~, posterior_probs_marg(:,idx_n,:)]=lda_classify(r_all(:,idx_n), s_all, [], train_idxs);
        end
        
        
        [~, posterior_probs_j_sh, ~,  K_sh, L_sh]=lda_classify(r_all_sh, s_all, [], train_idxs);
        posterior_probs_marg_sh = zeros(length(test_idxs),size(r_all,2),2);
        for idx_n = 1:size(r_all,2)
        [~, posterior_probs_marg_sh(:,idx_n,:)]=lda_classify(r_all_sh(:,idx_n), s_all, [], train_idxs);
        end
...
ps_marg = posterior_probs_marg(:,:,class1_idx);

ps_joint_sh = posterior_probs_j_sh(:,class1_idx);
ps_marg_sh = posterior_probs_marg_sh(:,:,class1_idx);
...
% region = zeros(size(ps_marg,1),1);
% region( sum(ps_marg>.5,2)==size(ps_marg,2) ) = 1;
% region( sum(ps_marg<.5,2)==size(ps_marg,2) ) = 2;
[region] = consistency_region_pairs(ps_marg,consistency_thr);
consistent = region == 1 | region == 2;
inconsistent = region == 0;
...
% region = zeros(size(ps_marg_sh,1),1);
% region( sum(ps_marg_sh>.5,2)==size(ps_marg_sh,2) ) = 1;
% region( sum(ps_marg_sh<.5,2)==size(ps_marg_sh,2) ) = 2;
[region] = consistency_region_pairs(ps_marg_sh,consistency_thr);
consistent_shuffle = region == 1 | region == 2;
inconsistent_shuffle = region == 0;
...
sp_corr = feval(f_db,r(:,1),r(:,2),r(:,3))>0;
sp_diag = feval(f_db_sh,r(:,1),r(:,2),r(:,3))>0;
...
[corr_coeff_BP_correct,corr_coeff_BP_error, n_all_selected_trials] = compute_correlation_coefficient_correct_error_N(s(selected),choice(selected),r(selected,1:N-1),r(selected,N-1:end));

out.corr_coeff_BP_correct = corr_coeff_BP_correct;
out.corr_coeff_BP_error = corr_coeff_BP_error;

[corr_coeff_BP_correct_subopt,corr_coeff_BP_error_subopt, n_all_selected_trials] = compute_correlation_coefficient_correct_error_N(s(selected),choice_subopt(selected),r(selected,1:N-1),r(selected,N-1:end));

out.corr_coeff_BP_correct_subopt = corr_coeff_BP_correct_subopt;
out.corr_coeff_BP_error_subopt = corr_coeff_BP_error_subopt;
