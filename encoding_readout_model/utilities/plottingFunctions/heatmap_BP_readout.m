function heatmap_BP_readout(data_folder, N, file_name, consistency_strength_idx, save_path, save_name, save_data, save_figs)
...
behav_perf_subopt = simu_out.behav_perf_subopt(:,:,:);

figure;
delta_BP_readout = cellfun(@minus,squeeze(behav_perf(consistency_strength_idx,:,:))',squeeze(behav_perf_subopt(consistency_strength_idx,:,:))','UniformOutput',false);
delta_BP_readout_mean = cellfun(@mean,delta_BP_readout);
clim=max(abs(delta_BP_readout_mean(:)));
imagesc(gamma_range,rho_range,delta_BP_readout_mean)
colormap(gca,myColorMapRedBlue_log);
set(gca,'YDir','reverse')
caxis([-clim clim]);
axis square; axis tight;
title('BP consEnhanced - consIndependent');
xlabel('\gamma');
if plot_Ni
...
save(fullfile(save_path, 'data_for_figures', ['DATA_' save_name readoutStr '.mat']),...
    'gamma_range','rho_range','delta_BP_readout_mean');
writematrix(delta_BP_readout_mean,fullfile(save_path, 'data_for_figures', ['DATA_' save_name readoutStr '.csv']));
end
cd(curr_dir);
